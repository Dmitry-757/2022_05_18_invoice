...
public class InvoicesUpLevelMenu {
    public static void workWithInvoices() {
        ...
        while (!goBack) {
            System.out.println("1 - New invoice, 2 - change invoice, 3 - Delete invoice, 4 - print one invoice, " +
                    "5 - print all invoices, 6 - printInvoicesByClient, 7 - go back");
            //sc.nextLine();
            if (sc.hasNextInt()) {
                ...
                switch (choice) {
                    case 1 -> {
                        try {
                            CreateNewInvoice.createOrChangeInvoice(null);
                        } catch (Exception e) {
    ...
    private static void changeInvoice() {
        ...
        if (matcher.find()) {
            ...
            for (Invoice invc : StoreService.getInvoiceByParam(invoiceNumber)) {
                System.out.println("1 - change head, 2 - change strings, 3 - go back");
                if (sc.hasNextInt()) {
                    int choice = sc.nextInt();
                    sc.nextLine();
                    switch (choice) {
                        case 1 -> {
                            try {
                                CreateNewInvoice.createOrChangeInvoice(invc);
                            } catch (Exception e) {
                                System.out.println(e.getMessage());
                            }
                        }
                        case 2 -> {
                            WorkWithStringsOfInvoice.workWithStringsOfInvoice(invc);
                        }
                    }
                }
    ...
    private static void deleteInvoice() {
        ...
        if (matcher.find()) {
            ...
            for (Invoice invc : StoreService.getInvoiceByParam(invoiceNumber)) {
                StoreService.deleteInvoice(invc);
            }
        } else {
            System.out.println("wrong input...");
        }
